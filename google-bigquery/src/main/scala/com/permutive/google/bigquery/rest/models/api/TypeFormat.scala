...
private[rest] object TypeFormat {
  ...
  @newtype case class Int64Value(value: Long)
  object Int64Value {
    implicit val decoder: Decoder[Int64Value] = deriveDecoder[String, Int64Value](s => Try(Int64Value(s.toLong)))
    implicit val encoder: Encoder[Int64Value] = deriveEncoder[String, Int64Value](_.value.toString)
  }
  ...
  @newtype case class UInt64Value(value: Long)
  object UInt64Value {
    implicit val decoder: Decoder[UInt64Value] = deriveDecoder[String, UInt64Value](s => Try(UInt64Value(s.toLong)))
    implicit val encoder: Encoder[UInt64Value] = deriveEncoder[String, UInt64Value](_.value.toString)
  }

  private def deriveDecoder[Base, T](f: Base => Try[T])(implicit d: Decoder[Base]): Decoder[T] =
    d.emapTry(f)

  private def deriveEncoder[Base, T](f: T => Base)(implicit e: Encoder[Base]): Encoder[T] =
    e.contramap(f)
