...
object JobConfigurationApi {
  ...
  implicit val decoder: Decoder[JobConfigurationApi] = new Decoder[JobConfigurationApi] {
    override def apply(c: HCursor): Either[DecodingFailure, JobConfigurationApi] =
      // Decode the indicator and if that is OK decode the body
      JobTypeIndicatorApi.decoder(c).flatMap {
        case JobTypeIndicatorApi(JobTypeIndicator.Copy)    => Right(JobConfigurationApi.Copy)
        case JobTypeIndicatorApi(JobTypeIndicator.Extract) => Right(JobConfigurationApi.Extract)
        case JobTypeIndicatorApi(JobTypeIndicator.Load)    => Right(JobConfigurationApi.Load)
        case JobTypeIndicatorApi(JobTypeIndicator.Query)   => Query.decoder(c)
        case JobTypeIndicatorApi(JobTypeIndicator.Unknown) => Right(JobConfigurationApi.Unknown)
      }
  }

  implicit val encoder: Encoder.AsObject[JobConfigurationApi] = new Encoder.AsObject[JobConfigurationApi] {
    override def encodeObject(jca: JobConfigurationApi): JsonObject =
      jca match {
        case q: Query => indicatorJson(JobTypeIndicator.Query).deepMerge(q.asJsonObject)
  ...
  private[this] object JobTypeIndicator extends Enum[JobTypeIndicator] {
    ...
    implicit val decoder: Decoder[JobTypeIndicator] = Circe.decodeCaseInsensitive(this)
