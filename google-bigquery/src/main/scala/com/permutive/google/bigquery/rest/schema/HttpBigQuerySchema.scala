...
class HttpBigQuerySchema[F[_]: HttpMethods: Logger](
  ...
  private def tablesUriPost[T: Encoder](dataset: DatasetId, body: T): Request[F] =
    POST(body.asJson, tablesUri(dataset))

  private def tablesUriPatch[T: Encoder](dataset: DatasetId, table: TableId, body: T): Request[F] =
    PATCH(body.asJson, tableUri(dataset, table))

  private def datasetPost(datasetId: DatasetId, requestBody: CreateDatasetRequestApi) =
  ...
  override def createTable(
    ...
    assertPartitioningValid(fields, partitioning) >>
      ...
      HttpMethods[F]
        .sendAuthorizedRequest[TableResponseApi](tablesUriPost(dataset, requestBody), "create table")
        .void
  }

  override def patchTable(tableId: TableId, datasetId: DatasetId, fields: NonEmptyList[Field]): F[Unit] = {
    ...
    Logger[F].debug(s"Updating fields in table $tableId: $requestBody") >>
      HttpMethods[F]
        .sendAuthorizedRequest[TableResponseApi](tablesUriPatch(datasetId, tableId, requestBody), "update table")
        .void
  }
  ...
  override def createView(
    ...
    Logger[F].debug(s"Creating view $name: $requestBody") >>
      HttpMethods[F]
        .sendAuthorizedRequest[TableResponseApi](tablesUriPost(dataset, requestBody), "create view")
        .void
  }
  ...
  override def listTablesAndViews(
    ...
    val uri     = UriUtils.uriWithPageToken(UriUtils.uriWithMaxResults(baseUri, maxResults), pageToken)

    for {
      _ <- Logger[F].debug(s"Retrieving tables in `$dataset`. Max results: $maxResults; Page token: $pageToken")
  ...
  override def listAllTablesAndViews(
    ...
    val fetch: Option[PageToken] => F[(ListTablesAndViewsResult, Option[PageToken])] =
      listTablesAndViews(dataset, paginationSettings.maxResultsPerPage, _).fproduct(_.nextPageToken)

    val res: F[(TablesAndViewsMetadata, Stream[F, DatasetObject])] =
      StreamUtils.unrollResults[F, ListTablesAndViewsResult, DatasetObject, TablesAndViewsMetadata, DatasetObject](
        fetch,
    ...
    Logger[F].debug(s"Retrieving all tables in `$dataset`. Pagination: $paginationSettings") >>
      res
  }
  ...
  private def assertPartitioningValid(fields: NonEmptyList[Field], partitioning: Option[Partitioning]): F[Unit] = {
