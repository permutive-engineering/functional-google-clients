...
object DatasetObject {
  ...
  private[rest] def fromResponse(response: ListTableResponseApi): Either[BigQueryException, DatasetObject] =
    response.`type` match {
      case TableObjectType.Table => readTable(response)
      case TableObjectType.View  => readView(response)
    }

  private def readTable(responseApi: ListTableResponseApi): Either[BigQueryException, Table] =
    Right(
      Table(
  ...
  private def readView(responseApi: ListTableResponseApi): Either[BigQueryException, View] = {
